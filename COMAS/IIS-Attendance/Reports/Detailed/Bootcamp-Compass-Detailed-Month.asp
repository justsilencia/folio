...
<%
	...
	strPage = "SELECT COUNT(DISTINCT m.dc_number) FROM master_roster m INNER JOIN attend_btu_compass a ON m.dc_number = a.dc_number " & _
...
<%
    ...
    strClassN = "SELECT ClassN = MIN(class_number) FROM attend_btu_compass " & _ 
        "WHERE class_date >= '"&Session("Month3")&"/1/"&Session("Year")&"'" & _
		"AND class_date <= '"&Session("Month3")&"/"&Session("Days3")&"/"&Session("Year")&"'" & _

        "UNION " & _

        "SELECT ClassN = MAX(class_number) FROM attend_btu_compass " & _ 
        "WHERE class_date >= '"&Session("Month3")&"/1/"&Session("Year")&"'" & _
		"AND class_date <= '"&Session("Month3")&"/"&Session("Days3")&"/"&Session("Year")&"'"
    
    Set rsClassN = Server.CreateObject("ADODB.Recordset")	
	rsClassN.Open strClassN, eduConn, adOpenForwardOnly, adLockReadOnly, adCmdText

    If Not rsClassN.EOF Then
        intClassFirst = rsClassN("ClassN")
    rsClassN.MoveNext
    End if
    
    If Not rsClassN.EOF Then
        intClassLast = rsClassN("ClassN")
    End if
...
%>
 ...
 <tr height=21 style='mso-height-source:userset;height:15.95pt'>
  ...
  <td class=xl7426149 colspan=7><%=rsTeacher("Class")%>&nbsp;(&nbsp;<%=intClassFirst%>-&nbsp<%=intClassLast%>) Compass 100 Live</td>
  <td>&nbsp;&nbsp;&nbsp;</td>
  <td>&nbsp;&nbsp;&nbsp;</td>
...
<%
	...
	strMember = "SELECT COUNT(hours) FROM attend_btu_compass a inner join class_assignment c on a.dc_number = c.dc_number" & _
                " WHERE Class_Date = '"&Session("Month3")&"/"&memCount&"/"&Session("Year")&"'"
...
%>
  <td class=xl8426149 style='border-left:none'>
	...
	strMember = "SELECT COUNT(hours) FROM attend_btu_compass a inner join class_assignment c on a.dc_number = c.dc_number" & _
                " WHERE Class_Date = '"&Session("Month3")&"/29/"&Session("Year")&"'"
...
%>
  ...
  <%
	...
	strMember = "SELECT COUNT(hours) FROM attend_btu_compass a inner join class_assignment c on a.dc_number = c.dc_number" & _
                " WHERE Class_Date = '"&Session("Month3")&"/30/"&Session("Year")&"'"
...
<%
	...
	strMember = "SELECT COUNT(hours) FROM attend_btu_compass a inner join class_assignment c on a.dc_number = c.dc_number" & _
                " WHERE Class_Date = '"&Session("Month3")&"/31/"&Session("Year")&"'"  
...
<%
	...
	If wDay <> "1" AND wDay <> "7" Then
		...
		strMember = "SELECT hours FROM attend_btu_compass WHERE Class_Date = '"&Session("Month3")&"/1/"&Session("Year")&"'"
...
<%
	...
	strMember = "SELECT hours FROM attend_btu_compass WHERE class_date = '"&Session("Month3")&"/"&tpCount&"/"&Session("Year")&"'"
...
<td class=xl8726149 style='border-top:none;border-left:none'>
 <%
	...
	strMember = "SELECT hours FROM attend_btu_compass WHERE Class_Date = '"&Session("Month3")&"/29/"&Session("Year")&"'"
...
<%
	...
	strMember = "SELECT hours FROM attend_btu_compass WHERE Class_Date = '"&Session("Month3")&"/30/"&Session("Year")&"'"
...
<%
	...
	strMember = "SELECT hours FROM attend_btu_compass WHERE Class_Date = '"&Session("Month3")&"/31/"&Session("Year")&"'"
...
<%
        ...
        strHoursQuery = "select hours from attend_btu_compass where class_date >= '"&Session("Month3")&"/1/"&Session("Year")&"' and class_date <= '"&Session("Month3")&"/"&Session("Days3")&"/"&Session("Year")&"'"
